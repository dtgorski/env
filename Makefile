.PHONY: help clean test bench prof-cpu prof-mem escape sniff tidy

GO_TEST     := CGO_ENABLED=1 go test -count=1 -race -v -coverprofile=./coverage.out
GO_BENCH    := CGO_ENABLED=0 go test -count=1 -benchmem -bench=.
GO_PROF_MEM := CGO_ENABLED=0 go test -count=1 -memprofile=mem.prof -bench=.
GO_PROF_CPU := CGO_ENABLED=0 go test -count=1 -cpuprofile=cpu.prof -bench=.
GO_HEAP_ESC := CGO_ENABLED=0 go build -gcflags "-m"
...
prof-mem: clean         # Creates memory profile (pick: ARGS="-bench=<Name>")
	$(GO_PROF_MEM) $(ARGS) .
	go tool pprof -top mem.prof | head -40

escape:                 # Displays heap escape analysis
	$(GO_HEAP_ESC) $(ARGS) . 2>&1 | grep -v "inlin" | grep -v "autogenerated" || true
